...
return view.extend({
	...
	renderSectionTLSConfigs: function(s) {
		...
		o = s.option(form.Value, "fake_sni_seq_len", _("Fake sni seq len"), _("This flag specifies ytunblock to build a complicated construction of fake client hello packets. length determines how much fakes will be sent."));
		o.depends("fake_sni", "1")
		o.default = 1;
		o.rmempty = false;
		...
		o = s.option(form.Flag, "frag_sni_reverse", _("Frag sni reverse"), _("Specifies ytunblock to send ClientHello fragments in the reverse order."));
		o.depends("frag", "tcp");
		o.depends("frag", "ip");
		o.enabled = '1'
		o.disabled = '0'
		...
		o = s.option(form.Flag, "frag_sni_faked", _("Frag sni faked"), _("Specifies ytunblock to send fake packets near ClientHello (fills payload with zeroes)."));
		o.depends("frag", "tcp");
		o.depends("frag", "ip");
		o.enabled = '1'
		o.disabled = '0'
		...
		o = s.option(form.Flag, "frag_middle_sni", _("Frag middle sni"), _("With this options ytunblock will split the packet in the middle of SNI data."));
		o.depends("frag", "tcp");
		o.depends("frag", "ip");
		o.enabled = '1'
		o.disabled = '0'
		...
		o = s.option(form.Value, "frag_sni_pos", _("Frag sni pos"), _("With this option ytunblock will split the packet at the position pos."));
		o.depends("frag", "tcp");
		o.depends("frag", "ip");
		o.rmempty = false;
		o.default = 1;

		o = s.option(form.Value, "seg2delay", _("seg2delay"), _("This flag forces ytunblock to wait a little bit before send the 2nd part of the split packet."));
		o.depends('tls_enabled', '1');
		o.default = 0;
	...
	renderSectionUDPConfigs: function(s) {
		...
		o = s.option(form.Flag, "quic_drop", _("QUIC drop"), _("Drop all QUIC packets which goes to ytunblock. Won't affect any other UDP packets. Please note, because of <code>--udp-mode=drop</code> it may conflict with other filter options. Make sure to use multiple sections."));
		o.enabled = '1'
		o.disabled = '0'
		o.default = o.disabled;
		o.rmempty = false;
	...
	renderGeneralConfigs: function(s) {
		...
		s.option(form.Value, "packet_mark", _("Packet mark"),
			_("Метка пакета при конфликте с другими системами (по умолчанию 32768)."));
		s.option(form.Value, "post_args", _("Post args"),
			_("Дополнительные аргументы командной строки."));
	},

	render: function(result) {
		let m, s, o;

		m = new form.Map('ytunblock', _('YT Unblock — Настройки'),
			_("Полная конфигурация DPI-обхода. Профили на вкладке «Обзор» перезаписывают эти параметры."));

		const general_section = m.section(form.NamedSection, "ytunblock", "ytunblock");

		o = general_section.option(form.Flag, "enabled", _("Включить службу"),
			_("Глобальное включение/выключение без удаления настроек."));
		o.default = '1';
		o.rmempty = false;

		o = general_section.option(form.ListValue, "conf_strat", _("Режим конфигурации"),
			_("«Флаги UI» — через форму; «Аргументы» — произвольная командная строка."));
		o.widget = "radio";
		o.value("args");
		o.value("ui_flags");
		o.default = "ui_flags";
		...
		o = general_section.option(form.SectionValue, "_flags_section", 
			form.NamedSection, "ytunblock", "ytunblock", _("UI Flags configuration"));
		...
		subsects_section.sectiontitle = function(section_id) {
			return uci.get('ytunblock', section_id, 'name') || _('Unnamed section');
		};
